...
from src.dataset_loader.OPTIC_dataloader import OPTIC_dataset
from src.dataset_loader.convert_csv_to_list import convert_labeled_list
from src.dataset_loader.optic_transform import collate_fn_wo_transform, collate_fn_w_transform
...
from src.dataset_loader.normalize import normalize_image
...
from src.common_utils.optic_metrics import calculate_metrics
...
def eval_model_oc_dc(experiment_name, segmentation_model, validate_loader, pred_masks_dir):
    segmentation_model.eval()
    metrics_y = [[], [], [], []]
    metric_dict = ['Disc_Dice', 'Disc_ASD', 'Cup_Dice', 'Cup_ASD']
    segmentation_model.train(if_testing=True)
    with torch.no_grad():
        for b_iter, test_batch in enumerate(tqdm(validate_loader)):
            ...
            x = makeVariable(clean_image_l, type='float', use_gpu=use_gpu, requires_grad=True)
            seg_logit = segmentation_model.predict(x, n_iter=2, normalize_input=False)
            seg_output = torch.sigmoid(seg_logit)
            metrics = calculate_metrics(seg_output.detach().cpu(), label_l.detach().cpu())
            for i in range(len(metrics)):
                metrics_y[i].extend(metrics[i])
            ...
            # Optic Cup
            cv2.imwrite(pred_masks_dir + '/' + img_name + '_OC.png', draw_output[0][1])
            # Optic Disc
            cv2.imwrite(pred_masks_dir + '/' + img_name + '_OD.png', draw_output[0][0])
...
def save_images(input_batch, output_batch, epoch, batch_index, save_dir='output_images'):
    ...
    for i in range(input_batch.size(0)):
        ...
        # Convert the input image to numpy format (Assuming RGB format: [C, H, W])
        input_image = np.clip(input_batch[i].cpu().permute(1, 2, 0).numpy(), 0, 1)
        axes[0].imshow(input_image)
        axes[0].set_title('Input Image')
        axes[0].axis('off')

        # Convert the output image to numpy format
        output_image = output_batch[i].cpu().permute(1, 2, 0).numpy()
        ...
        # Save the plot
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f'epoch_{epoch}_batch_{batch_index}_image_{i}.png'))
        plt.close(fig)
...
def train_network(
    ...
    '''
    # output setting
    global start_epoch, last_epoch, training_opt, crop_size, model_dir, log_dir, dataset_name, generate_images_dir
    ...
    train_loader = DataLoader(
        dataset=train_set,
        num_workers=0,  # int(training_opt.n_workers),
        batch_size=int(train_batch_size),
        shuffle=True,
        drop_last=True,
        pin_memory=False,  # not training_opt.no_pin_memory,
        worker_init_fn=seed_worker,
        generator=g,
        collate_fn=collate_fn_w_transform if dataset_name == "OD-OC-Segmentation" else None
    )
    validate_loader = DataLoader(
        ...
        worker_init_fn=seed_worker,
        generator=g,
        collate_fn=collate_fn_wo_transform if dataset_name == "OD-OC-Segmentation" else None
    )
    ...
    if apply_nflows_mixing:
        loss_keys += [
            "loss/hard_nflows/total",
            "loss/hard_nflows/seg",
            "loss/hard_nflows/image",
            "loss/hard_nflows/shape"
        ]
    loss_dict = {}
    ...
    try:
        for i_epoch in range(start_epoch, experiment_opt['learning']['n_epochs']):
            ...
            for i_iter, sampled_batch in enumerate(tqdm(train_loader)):
                ...
                # add noise to input to train the FTN
                # (same as training a standard denoising autoencoder to avoid overfitting)
                noise = 0.05 * torch.randn(
                    batch_4d_size[0],
                    batch_4d_size[1],
                    batch_4d_size[2],
                    batch_4d_size[3],
                    device=clean_image_l.device,
                    dtype=clean_image_l.dtype
                )
                image_l = clean_image_l + noise
                if intensity_norm_type == 'min_max':
                    image_l = torch.clamp(image_l, clean_image_l.min(), clean_image_l.max())
                elif intensity_norm_type == 'z_score':
                    image_l = F.instance_norm(image_l, eps=1e-05, momentum=0.1, weight=None, bias=None)
                else:
                    raise ValueError('intensity_norm_type not supported')
                image_l = makeVariable(image_l, use_gpu=use_gpu, requires_grad=False, type='float')

                # step 2: standard training
                seg_loss, image_recon_loss, gt_recon_loss, shape_recon_loss, easy_recon_image, p0, p_refine = (
                    segmentation_solver.standard_training(
                        clean_image_l,
                        label_l,
                        perturbed_image=image_l,
                        return_output=True
                    )
                )
                ...
                if latent_DA:
                    ...
                    perturbed_image_0, perturbed_y_0 = segmentation_solver.hard_example_generation(
                        clean_image_l.detach().clone(),
                    ...
                    seg_supervised_loss, corrupted_image_recon_loss, shape_recon_loss_2, corrupted_shape_recon_loss = (
                        segmentation_solver.hard_example_traininng(perturbed_image=perturbed_image_0,
                                                                   perturbed_seg=perturbed_y_0,
                                                                   clean_image_l=clean_image_l, label_l=label_l,
                                                                   standard_input_image=image_l.detach().clone(),
                                                                   standard_recon_image=easy_recon_image))
                ...
                if apply_nflows_mixing:
                    segmentation_solver.reset_all_optimizers()
                    stylized_images = segmentation_solver.generate_nflows_mixing_images(
                        image_code=z_i
                    )

                    # Save input and output images
                    if (i_epoch + 1) % experiment_opt["output"]["save_pics_every_num_epochs"] == 0:
                        save_images(image_l, stylized_images, i_epoch + 1, i_iter, generate_images_dir)

                    l_seg_1, l_rec, l_shape_1, l_shape_2 = segmentation_solver.hard_example_traininng(
                        perturbed_image=stylized_images,
                        perturbed_seg=None,
                        clean_image_l=clean_image_l,
                        label_l=label_l,
                        standard_input_image=image_l.detach().clone(),
                        standard_recon_image=easy_recon_image
                    )
                    nflows_mixing_loss = l_rec + l_seg_1 + l_shape_1 + l_shape_2
                    loss_dict['loss/hard_nflows/total'] += nflows_mixing_loss.item()
                    loss_dict['loss/hard_nflows/seg'] += l_seg_1.item()
                    loss_dict['loss/hard_nflows/image'] += l_rec.item()
                    loss_dict['loss/hard_nflows/shape'] += (l_shape_1 + l_shape_2).item()

                else:
                    nflows_mixing_loss = torch.tensor(0., device=device)
                ...
                if max_style:
                    ...
                    if dataset_name == 'OD-OC-Segmentation':
                        channel_num[-1] = 3
                    stylized_images = segmentation_solver.generate_max_style_image(
                        image_code=z_i,
                        channel_num=channel_num,
                        p=0.5,
                        decoder_layers_indexes=experiment_opt['max_style']['decoder_layers_indexes'],
                ...
                loss = (standard_loss + LDA_loss + max_style_loss +
                        rand_conv_loss + RSC_loss + mix_style_loss +
                        DSU_loss + adv_noise_loss + adv_bias_loss +
                        nflows_mixing_loss)
                segmentation_solver.reset_all_optimizers()
                loss.backward()
                segmentation_solver.optimize_all_params()
                total_loss += loss.item()
                total_nflows_mixing_loss += nflows_mixing_loss.item()
                torch.cuda.empty_cache()
                if log:
                ...
                #     stop_flag = True
            print('{} network: {} epoch {} training loss iter: {}, total  loss: {}, nflow_loss: {}'.
                  format(experiment_name, experiment_opt['segmentation_model']["network_type"], i_epoch, i_iter,
                         str(total_loss / (1.0 * g_count)), str(total_nflows_mixing_loss / (1.0 * g_count))))
            if segmentation_solver.schedulers_dict is not None:
                segmentation_solver.update_schedule()
                for module_name, optimizer in segmentation_solver.optimizers.items():
                    writer.add_scalar(f'lr/{module_name}', optimizer.param_groups[0]['lr'], i_epoch)
                    print(f"Current learning rate for {module_name}: {optimizer.param_groups[0]['lr']:.16f}")
 
            # =========================<<<<<start evaluating>>>>>>>>=============================
            if (i_epoch + 1) % experiment_opt["output"]["eval_epoch_every_num_epochs"] == 0:
                if dataset_name != 'OD-OC-Segmentation':
                    ...
                    test_metric = eval_model_oc_dc(experiment_name, segmentation_solver, validate_loader, pred_masks_dir)
                    if log:
                        writer.add_scalar('dices/test_disc_dice', test_metric['Disc_Dice'], i_epoch)
                        writer.add_scalar('dices/test_cup_dice', test_metric['Cup_Dice'], i_epoch)
                        writer.add_scalar('asds/test_disc_asd', test_metric['Disc_ASD'], i_epoch)
                        writer.add_scalar('asds/test_cup_asd', test_metric['Cup_ASD'], i_epoch)
                    for metric, value in test_metric.items():
                        print(f"EP {i_epoch+1} - Eval: {metric}: {value:.3f}")
                    curr_score = float(test_metric['Disc_Dice']) + float(test_metric['Cup_Dice'])
                # save best models
...
if __name__ == '__main__':
    ...
    if not training_opt.no_train:
        ...
        if 'ACDC' in dataset_name:
            ...
            if isinstance(frame, list):
                ...
                frame_list = [frame]
            assert len(
                frame_list) <= 2, ('currently, only support concat two sets, '
                                   'please check your [frame] in the config file')
            train_set_list = []
            validate_set_list = []
            ...
            for a_frame in frame_list:
                ...
                train_set = Cardiac_General_Dataset(root_dir=join(root_folder, 'train'),
                                                    ...
                                                    crop_size=data_opt['crop_size'],
                                                    normalize=True,
                                                    # disable it when data has been preprocessed to save time                                            intensity_norm_type = intensity_norm_type
                                                    intensity_norm_type=intensity_norm_type
                                                    )
        ...
        elif 'OD-OC-Segmentation' in dataset_name:
            src_csv_list = []
            for src_name in data_opt["source_datasets"]:
                src_csv_list.append(f"{src_name}_train.csv")
                src_csv_list.append(f"{src_name}_test.csv")
            sr_img_list, sr_label_list = convert_labeled_list(data_opt["root_dir"], src_csv_list)
            #sr_img_list = sr_img_list[:10]
            #sr_label_list = sr_label_list[:10]

            train_set = OPTIC_dataset(
                data_opt["root_dir"],
                sr_img_list,
                sr_label_list,
                data_opt["image_size"][0],
                img_normalize=False
            )

            trgt_csv_list = []
            for trgt_name in data_opt["target_datasets"]:
                trgt_csv_list.append(f"{trgt_name}_train.csv")
                trgt_csv_list.append(f"{trgt_name}_test.csv")
            ts_img_list, ts_label_list = convert_labeled_list(data_opt["root_dir"], trgt_csv_list)
            #ts_img_list = ts_img_list[:10]
            #ts_label_list = ts_label_list[:10]
            validate_set = OPTIC_dataset(
                data_opt["root_dir"],
                ts_img_list,
                ts_label_list,
                data_opt["image_size"][0],
    ...
    lr_scheduler_name = get_value_from_dict(experiment_opt['learning'], "lr_scheduler", None)
    ...
    apply_nflows_mixing = get_value_from_dict(experiment_opt['learning'], "apply_nflows_mixing", False)
    load_weights = get_value_from_dict(experiment_opt['learning'], "load_pretrained_model", True)
    decoder_layers_indexes = experiment_opt['nflows_model']["decoder_layers_indexes"] if apply_nflows_mixing else None
    mean_weights_format_name = experiment_opt['nflows_model'][
        "mean_weights_format_name"] if apply_nflows_mixing else None
    var_weights_format_name = experiment_opt['nflows_model']["var_weights_format_name"] if apply_nflows_mixing else None
    prob = get_value_from_dict(experiment_opt['nflows_model'], "prob", 1.0)

    segmentation_solver = AdvancedTripletReconSegmentationModel(
        network_type=network_type,
        image_size=data_opt['crop_size'][0],
        image_ch=3 if "OD-OC-Segmentation" in dataset_name else 1,
        ...
        intensity_norm_type=intensity_norm_type,
        dataset_name=dataset_name,
        apply_nflows_mixing=apply_nflows_mixing and not training_opt.no_train,
        load_weights=load_weights,
        decoder_layers_indexes=decoder_layers_indexes,
        mean_weights_format_name=mean_weights_format_name,
        var_weights_format_name=var_weights_format_name,
        prob=prob,
        data_ratio=training_opt.data_setting,
        src_domain=data_opt["source_datasets"][0],
        num_epochs=int(experiment_opt['learning']['n_epochs'])
    )
    ...
    generate_images_dir = join(global_dir, *[project_str, experiment_name, 'generated_images'])
    pred_masks_dir = join(global_dir, *[project_str, experiment_name, 'pred_masks'])
    check_dir(log_dir, create=True)
    check_dir(model_dir, create=True)
    check_dir(pred_masks_dir, create=True)
    # copy  config file to saved folder
    saved_config_path = join(global_dir, *[project_str, experiment_name, 'config.json'])
    counter = 1
    while os.path.exists(saved_config_path):
        saved_config_path = f"{saved_config_path}_{counter}"
        counter += 1
    ...
    if training_opt.auto_test:
        ...
        if training_opt.resume_pkl_path is not None or training_opt.test_model_dir_path is not None:
            assert (
                               training_opt.resume_pkl_path is not None and training_opt.test_model_dir_path is not None) is False, "only resume_pkl_path or test_model_dir_path need to be specified"
            if training_opt.resume_pkl_path is not None:
                start_epoch = segmentation_solver.load_snapshots(training_opt.resume_pkl_path)
        ...
        for test_dataset_name, test_root_dir in zip(test_dataset_name_list,test_root_dir_list):
            ...
            means, stds, concatenated_df = evaluate_fn(
                segmentation_model=segmentation_solver,
                test_dataset_name=test_dataset_name,
                test_root_dir=test_root_dir,
                test_set_ratio=1.0,
                method_name=config_name,
                maximum_batch_size=training_opt.test_batch_size,
                normalize_2D=True,
                new_spacing=data_opt['new_spacing'],
                crop_size=data_opt["crop_size"],
