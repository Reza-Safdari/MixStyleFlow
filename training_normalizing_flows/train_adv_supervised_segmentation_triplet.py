...
def train_network(
    ...
    train_loader = DataLoader(
        dataset=train_set,
        num_workers=0, # training_opt.n_workers,
    ...
    if nflows_training:
        loss_keys += ["loss/nflows/mean", "loss/nflows/var"]
        ...
        for i_epoch in range(start_epoch, experiment_opt['learning']['n_epochs']):
            ...
            for i_iter, sampled_batch in enumerate(tqdm(train_loader)):
                ...
                if not nflows_training:
                    ...
                    mean_nflow_loss = torch.tensor(0., device=device)
                    var_nflow_loss = torch.tensor(0., device=device)
                elif nflows_training:
                    mean_nflow_loss, var_nflow_loss, mu, var = segmentation_solver.train_nflows_models(
                        perturbed_image=image_l,
                        decoder_layers_indexes=mixstyle_layer
                    )
                    loss_dict['loss/nflows/mean'] += mean_nflow_loss.item()
                    loss_dict['loss/nflows/var'] += var_nflow_loss.item()
                    standard_loss = torch.tensor(0., device=device)
                    if (i_epoch + 1) % experiment_opt["output"]["save_stats_every_num_epochs"] == 0:
                        stats.extend((m.cpu().numpy(), v.cpu().numpy()) for m, v in zip(mu, var))
                ...
                loss = (standard_loss + LDA_loss + max_style_loss +
                        ...
                        mean_nflow_loss + var_nflow_loss)
                segmentation_solver.reset_all_optimizers()
                loss.backward()
                if grad_clip_norm != 0 and nflows_training:
                    # print("I am here!!!!")
                    torch.nn.utils.clip_grad_norm_(segmentation_solver.mean_nflows_model.parameters(),
                                                   max_norm=grad_clip_norm,
                                                   error_if_nonfinite=True)
                    torch.nn.utils.clip_grad_norm_(segmentation_solver.var_nflows_model.parameters(),
                                                   max_norm=grad_clip_norm,
                                                   error_if_nonfinite=True)
            ...
            if (i_epoch + 1) % experiment_opt["output"]["save_stats_every_num_epochs"] == 0:
                save_path = '/'.join(model_dir.split('/')[:-1])
                np.savez(os.path.join(save_path, f"means_vars_epotch_{i_epoch + 1}_{dataset_name}_L{mixstyle_layer}.npz"), *stats)
...
class NFlowConfig:
    def __init__(self, nflow_configs):
        self.latent_size = nflow_configs["latent_size"]
        self.hidden_size = nflow_configs["hidden_size"] if "hidden_size" in nflow_configs.keys() else None
        self.n_flows = nflow_configs["n_flows"]
        self.clamp_value = nflow_configs["clamp_value"] if nflow_configs["clamp_value"] != "None" else None
        self.normalize_input = nflow_configs["normalize_input"]


def get_nflow_config(configs):
    # if not configs["learning"]["apply_nflows"]:
    #     return None
    return NFlowConfig(configs["nflows_model"])
...
if __name__ == '__main__':
    ...
    nflows_training = True if "nflows_model" in experiment_opt.keys() else False
    grad_clip_norm = experiment_opt['learning']["grad_clip_norm"] if nflows_training else 5.0
    if "nflows_model" in experiment_opt.keys():
        mixstyle_layer = experiment_opt["nflows_model"]["mixstyle_layer"]
    else:
        mixstyle_layer = []
    ...
    segmentation_solver = AdvancedTripletReconSegmentationModel(
        ...
        use_gpu=use_gpu,
        n_iter=1,
        checkpoint_dir=training_opt.test_model_dir_path if nflows_training else None,
        ...
        intensity_norm_type=intensity_norm_type,
        dataset_name=dataset_name,
        nflow_config=get_nflow_config(experiment_opt) if nflows_training else None
    )
    ...
    if not training_opt.no_train:
        ...
        train_network(
            ...
            train_nflows=nflows_training,
            grad_clip_norm=grad_clip_norm,
            mixstyle_layer=mixstyle_layer
        )
        # except:
    ...
    if training_opt.auto_test:
        if nflows_training:
            training_opt.test_model_dir_path = None
        # test
