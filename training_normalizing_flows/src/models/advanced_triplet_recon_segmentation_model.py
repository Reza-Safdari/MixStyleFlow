...
class AdvancedTripletReconSegmentationModel(nn.Module):
    def __init__(self, network_type='FCN_16_standard', image_ch=1,
        ...
        # initialization
        if nflow_config is not None:
            assert checkpoint_dir is not None, "When nflows models are training 'checkpoint_dir' can not be None."
        self.model = self.get_network(checkpoint_dir=checkpoint_dir)
        self.mean_nflows_model = get_normalizing_flow(nflow_config,
                                                      self.use_gpu) if self.nflow_config is not None else None
        self.var_nflows_model = get_normalizing_flow(nflow_config,
                                                     self.use_gpu) if self.nflow_config is not None else None
        self.temperature = 2
    ...
    def train_nflows_models(self, perturbed_image, decoder_layers_indexes=[3, 4, 5]):

        for name, module in self.model.items():
            set_grad(module, requires_grad=False)

        with torch.no_grad():
            latent_code_i, latent_code_s = self.encode_image(perturbed_image, disable_track_bn_stats=True)
            decoder = self.model["image_decoder"]
            with _disable_tracking_bn_stats(decoder):
                features = decoder(latent_code_i, decoder_layers_indexes)
                feature_maps = features[0].detach().clone()
        mu = feature_maps.mean(dim=[2, 3])
        var = feature_maps.var(dim=[2, 3])
        mean_nflow_loss = self.mean_nflows_model.forward_kld(mu)
        var_nflow_loss = self.var_nflows_model.forward_kld(var)
        return mean_nflow_loss, var_nflow_loss, mu, var
    ...
    def save_model(self, save_dir, epoch_iter, model_prefix=None, save_optimizers=False):
        ...
        if self.nflow_config is not None:
            torch.save(self.mean_nflows_model.state_dict(),
                       join(epoch_path, 'mean_nflows_model.pth'))
            torch.save(self.var_nflows_model.state_dict(),
                       join(epoch_path, 'var_nflows_model.pth'))
    ...
    def set_optimizers(self):
        ...
        if self.nflow_config is None:
            ...
            if self.optimizer_type == 'AdamW':
                assert self.mean_nflows_model is not None, "Mean NFlows model can not be None"
                assert self.var_nflows_model is not None, "Var NFlows model can not be None"
                if self.optimizer_type == 'AdamW':
                    optimizers_dict["mean_nflows"] = optim.AdamW(
                        self.mean_nflows_model.parameters(),
                        lr=self.learning_rate
                    )
                    optimizers_dict["var_nflows"] = optim.AdamW(
                        self.var_nflows_model.parameters(),
                        lr=self.learning_rate
                    )
                else:
                    raise NotImplementedError
