...
class ProstateDataset(BaseSegDataset):
    ...
    def get_pid_list(self, identifier, cval):
        ...
        train_val_ids = sorted(list(set(all_p_id_list) - set(test_ids)))
        ...
        if identifier == 'all':
            ...
            if 0 < identifier < 1:
                labelled_ids_split, _ = train_test_split(train_ids, train_size=identifier, random_state=cval)
            elif identifier > 1:
            ...
            labelled_ids_split = labelled_ids
        print(f"Validation set: {val_ids} $ Length={len(val_ids)}")
        print(f"Training set: {labelled_ids_split} $ Length={len(labelled_ids_split)}")
        print(f"Test ids: {test_ids} $ Length={len(test_ids)}")
        return {
