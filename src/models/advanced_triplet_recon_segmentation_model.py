...
from src.models.custom_loss import Seg_loss
...
from src.common_utils.schedulers import EpochLR
...
from src.models.nflows import get_normalizing_flow
import numpy as np
...
class NFlowConfig:
    def __init__(self, nflow_configs):
        ...
        # self.prob = nflow_configs["prob"]
        self.temp = nflow_configs["temperature"]


def get_nflow_config(configs):
    return NFlowConfig(configs)


class AdvancedTripletReconSegmentationModel(nn.Module):
    def __init__(self, network_type='FCN_16_standard',
                 image_ch=1,
                 learning_rate=1e-4,
                 ...
                 dataset_name="",
                 apply_nflows_mixing=False,
                 load_weights=False,
                 decoder_layers_indexes=None,
                 mean_weights_format_name=None,
                 var_weights_format_name=None,
                 prob=1.0,
                 data_ratio="all",
                 src_domain="",
                 num_epochs=600
        ...
        self.apply_nflows_mixing = apply_nflows_mixing
        self.decoder_layers_indexes = decoder_layers_indexes
        self.mean_weights_format_name = mean_weights_format_name
        self.var_weights_format_name = var_weights_format_name
        ...
        self.style_means_models = dict()
        self.style_vars_models = dict()
        ...
        if self.apply_nflows_mixing:
            for lyr_num, config in self.decoder_layers_indexes.items():
                model_config = get_nflow_config(config)
                self.style_means_models[int(lyr_num)] = get_normalizing_flow(model_config, self.use_gpu)
                self.style_vars_models[int(lyr_num)] = get_normalizing_flow(model_config, self.use_gpu)

                mean_weight_path = os.path.join(
                    checkpoint_dir,
                    self.mean_weights_format_name.format(epoch_num=200, lyr_num=lyr_num, src_domain=src_domain)
                )
                self.style_means_models[int(lyr_num)].load_state_dict(torch.load(mean_weight_path))

                var_weight_path = os.path.join(
                    checkpoint_dir,
                    self.var_weights_format_name.format(epoch_num=200, lyr_num=lyr_num, src_domain=src_domain)
                )
                self.style_vars_models[int(lyr_num)].load_state_dict(torch.load(var_weight_path))

                print(f"Weights of style mean and variance nflows models  are loaded from:")
                print(mean_weight_path)
                print(var_weight_path)
                print()
        else:
            self.mean_nflows_model = None
            self.var_nflows_model = None
        ...
        self.schedulers_dict = self.set_schedulers(lr_scheduler_name)
    ...
    def get_network(self, checkpoint_dir=None, load_weights=True, prob=1.0):
        '''
        ...
        if network_type in ['FCN_64_standard_no_STN',
                            'FCN_16_no_STN',
                            'FCN_16_standard_no_STN',
                            'FCN_16_standard_no_STN_no_im_recon',
                            'FCN_16_no_im_recon',
                            'FCN_16_standard_w_dual_image',
                            'FCN_16_standard_w_recon_image',
                            ...
                            'FCN_16_standard_no_STN_RESNET34']:
            ...
            use_resnet = True if "RESNET34" in network_type else False

            # FTN
            if 'DS_FCN' in network_type:
                image_encoder = Dual_Branch_Encoder(
                    input_channel=self.image_ch,
                    z_level_1_channel=512 // reduce_factor,
                    z_level_2_channel=512 // reduce_factor,
                    feature_reduce=reduce_factor,
                    if_SN=False,
                ...
                image_encoder = Dual_Branch_Encoder(
                    input_channel=self.image_ch,
                    z_level_1_channel=512 // reduce_factor if not use_resnet else 256,  # must be 256 for ResNet34
                    z_level_2_channel=512 // reduce_factor if not use_resnet else 256,  # must be 256 for ResNet34
                    feature_reduce=reduce_factor,
                    if_SN=False,
                    encoder_dropout=self.encoder_dropout,
                    norm=nn.BatchNorm2d,
            ...
            segmentation_decoder = MyDecoder(
                input_channel=512 // reduce_factor if not use_resnet else 256,  # must be 256 for ResNet34
                up_type='NN',
                output_channel=self.num_classes,
                feature_reduce=reduce_factor,
                if_SN=False,
            ...
            if not 'no_im_recon' in network_type:
                if 'NN_decoder' in network_type:
                    model_dict['image_decoder'] = MyDecoder(
                        input_channel=512 // reduce_factor,
                        up_type='NN',
                        output_channel=self.image_ch,
                        feature_reduce=reduce_factor,
                        if_SN=False,
                        decoder_dropout=self.decoder_dropout,
                        norm=nn.BatchNorm2d,
                        last_act=image_decoder_last_act
                    )
            ...
            if 'enable_code_filter' in network_type:
                ...
                enable_code_filter = False

            if 'UnetTransformer' in network_type:
                print('use UnetTransformer for image segmentation,enable_code_filter:', enable_code_filter)
                image_encoder = UNETR_Encoder(
                    in_channels=self.image_ch,
                    img_size=self.image_size,
                    feature_size=64 // reduce_factor,
                    hidden_size=hidden_size,
                    norm_name='batch',
                    spatial_dims=2,
                    enable_code_filter=enable_code_filter
                )
                segmentation_decoder = UNETR_Decoder(
            ...
            if not 'no_im_recon' in network_type:
                if not 'Unet_im_recon' in network_type:
                    ...
                    if 'UnetTransformer' in network_type:
                        ...
                        model_dict['image_decoder'] = UNETR_Decoder(
                            out_channels=self.image_ch,
                            feature_size=64 // reduce_factor,
                            hidden_size=hidden_size,
                            norm_name='batch',
                            spatial_dims=2,
                        ...
                        model_dict['image_decoder'] = UnetDecoder(
                            n_classes=self.image_ch,
                            reduce_factor=reduce_factor,
                            decoder_dropout=self.decoder_dropout,
                            norm=nn.BatchNorm2d, up_type='Conv2',
                            last_act=self.last_act,
                            activation=image_decoder_last_act
                        )
            shape_encoder, shape_decoder = self.build_shape_encoder_decoder(network_type)
            if shape_decoder is not None:
                model_dict['shape_decoder'] = shape_decoder
        ...
        checkpoint_dir = checkpoint_dir if load_weights else None
        model_dict = self.init_model_with_pretrained(model_dict, checkpoint_dir, use_resnet)
    ...
    def init_model(self, model, resume_path=None):
        if not resume_path is None:
            ...
            try:
                ...
                print('init network with kaiming')
            except:
                print('failed to init model')
        return model

    def init_model_with_pretrained(self, model_dict, checkpoint_dir, use_resnet=False):
        # load weights for each module
        for module_name, module in model_dict.items():
            if use_resnet and module_name == 'image_encoder':
                continue
    ...
    def generate_nflows_mixing_images(self, image_code):
        decoder = self.model["image_decoder"]
        num_samples = image_code.size(0)

        style_stats = defaultdict(list)
        for lyr_num in self.decoder_layers_indexes:
            style_stats[int(lyr_num)].extend(
                [
                    self.style_means_models[int(lyr_num)].get_sample(num_samples)[0].unsqueeze(-1).unsqueeze(-1),
                    self.style_vars_models[int(lyr_num)].get_sample(num_samples)[0].unsqueeze(-1).unsqueeze(-1),
                    np.random.rand(1)
                ]
            )

        # sampled_mean, _ = self.mean_nflows_model.get_sample(num_samples)
        # sampled_var, _ = self.var_nflows_model.get_sample(num_samples)
        recon_image = decoder.apply_nflows_mixing(
            image_code,
            style_stats
        )
        return recon_image.detach().clone()

    def generate_max_style_image(self,
                                 image_code,
                                 decoder_layers_indexes=[3, 4, 5],
                                 channel_num=[128, 64, 32, 16, 16, 1],
                                 p=0.5,
                                 n_iter=5,
                                 mix_style=True,
                                 lr=0.1, no_noise=False,
                                 reference_image=None,
                                 reference_segmentation=None,
                                 noise_learnable=True,
                                 mix_learnable=True,
                                 loss_types=['seg'],
                                 loss_weights=[1],
                                 always_use_beta=False,
                                 debug=False,
                                 fix_seed=None
                                 ):
        """_summary_
        ...
        if not len(decoder_layers_indexes) > 0:
           recon_image = self.decoder_inference(
               decoder_name='image_decoder',
               latent_code=image_code,
            ...
            if n_iter > 0:
                if len(list(nn_style_augmentor_dict.parameters())) == 0:
                    ...
                    assert reference_image is not None and reference_segmentation is not None, \
                        'must provide reference images and segmentations'
            ...
            for i in range(n_iter + 1):
                ...
                if i > 0:
                    ...
                    p = self.decoder_inference(
                        decoder=self.model['segmentation_decoder'],
                        latent_code=latent_code_s,
                        eval=False,
                    ...
                    for l_w, ltype in zip(loss_weights, loss_types):
                        if ltype == 'seg':
                            l = -basic_loss_fn(
                                pred=p,
                                target=reference_segmentation,
                                loss_type='cross_entropy_dice',  #'cross entropy',
                ...
                recon_image = decoder_function.apply_max_style(
                    image_code,
                    decoder_layers_indexes=decoder_layers_indexes,
                    nn_style_augmentor_dict=nn_style_augmentor_dict
                )
            for name, module in self.model.items():
    ...
    def standard_training(self,
                          ...
                          label_l,
                          perturbed_image,
                          compute_gt_recon=True,
                          update_latent=True,
                          if_latent_code_consistency=False,
                          disable_track_bn_stats=False,
                          domain_id=0,
                          return_output=False
                          ):
        ...
        if update_latent:
            ...
            self.z_s = z_s
        # seg task loss
        if self.dataset_name != 'OD-OC-Segmentation':
            seg_loss = cross_entropy_2D(y_0, label_l.detach(), weight=self.class_weights)
        else:
            seg_loss = self.Seg_loss(y_0, label_l.detach().float())
    ...
    def hard_example_traininng(self, perturbed_image, clean_image_l, perturbed_seg, label_l, use_gpu=True, if_latent_code_consistency=False, standard_input_image=None, standard_recon_image=None):
        ...
        if perturbed_image is not None:
            ...
            seg_loss, recon_loss, _, shape_loss = self.standard_training(
                clean_image_l=clean_image_l,
                label_l=label_l,
                perturbed_image=perturbed_image,
                compute_gt_recon=False,
                update_latent=False,
                disable_track_bn_stats=disable_track_bn_stats,
                domain_id=domain_id
            )
        ...
        if not 'no_STN' in self.network_type:
            if perturbed_seg is not None:
                perturbed_p_recon = self.recon_shape(perturbed_seg, image=standard_input_image,
                                                     recon_image=standard_recon_image,
    ...
    def set_schedulers(self, lr_scheduler_name):
        schedulers_dict = {}
        if lr_scheduler_name == 'Epoch':
            for k, v in self.optimizers.items():
                schedulers_dict[k] = EpochLR(v, epochs=self.num_epochs, gamma=0.9)
        else:
            schedulers_dict = None

        return schedulers_dict
