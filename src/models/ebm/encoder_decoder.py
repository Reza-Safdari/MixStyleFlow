...
from src.models.ebm.resnet import resnet34
...
class MyEncoder_ResNet(nn.Module):
    def __init__(self, input_channel, output_channel=None, feature_reduce=1, encoder_dropout=None, norm=nn.InstanceNorm2d, if_SN=False, act=torch.nn.Sigmoid()):
        super(MyEncoder_ResNet, self).__init__()
        self.encoder = resnet34(pretrained=True)

    def forward(self, x, fake):
        if x.size(1) == 1:
            x = x.repeat(1, 3, 1, 1)
        _, sfs = self.encoder(x)
        return sfs[-2]
...
class MyDecoder(nn.Module):
    ...
    def nflow_mix_styles(self, features, nn_style_augmentor, eps=1e-8):
        prob = nn_style_augmentor[-1]
        if prob > self.prob:
            return features
        ...
        lmda = self.beta.sample((B, 1, 1, 1)).to(features.device)
        # clipped_lmda = torch.clamp(lmda, 0, 1)
        # bernoulli = torch.bernoulli(lmda).to(features.device)
        # lmda *= bernoulli
        # lmda.requires_grad = False

        sampled_mu, sampled_var, _ = nn_style_augmentor
        sampled_mu = sampled_mu.clamp(min=eps)
        sampled_sig = sampled_var.clamp(min=eps).sqrt()

        mu_mix = mu * (1. - lmda) + sampled_mu * lmda
        sig_mix = sig * (1. - lmda) + sampled_sig * lmda

        return features_normed * sig_mix + mu_mix

    def apply_nflows_mixing(self, image_code, nn_style_augmentor_dict):
        if 0 in nn_style_augmentor_dict.keys():
            x = self.nflow_mix_styles(image_code.detach().clone(), nn_style_augmentor_dict[0])
        else:
            x = image_code.detach().clone()

        with _disable_tracking_bn_stats(self.up1):
            x2 = self.up1(x)
        if 1 in nn_style_augmentor_dict.keys():
            x2 = self.nflow_mix_styles(x2, nn_style_augmentor_dict[1])

        with _disable_tracking_bn_stats(self.up2):
            x3 = self.up2(x2)
        if 2 in nn_style_augmentor_dict.keys():
            x3 = self.nflow_mix_styles(x3, nn_style_augmentor_dict[2])

        with _disable_tracking_bn_stats(self.up3):
            x4 = self.up3(x3)
        if 3 in nn_style_augmentor_dict.keys():
            x4 = self.nflow_mix_styles(x4, nn_style_augmentor_dict[3])

        with _disable_tracking_bn_stats(self.up4):
            x5 = self.up4(x4)
        if 4 in nn_style_augmentor_dict.keys():
            x5 = self.nflow_mix_styles(x5, nn_style_augmentor_dict[4])
        ...
        if 5 in nn_style_augmentor_dict.keys():
            x5 = self.nflow_mix_styles(x5, nn_style_augmentor_dict[5])
        return x5
...
class Dual_Branch_Encoder(nn.Module):
    ...
    def __init__(self, input_channel, z_level_1_channel=None, z_level_2_channel=None, feature_reduce=1, encoder_dropout=None, norm=nn.InstanceNorm2d, if_SN=False, num_domains=1, use_resnet=False):
        ...
        if num_domains > 1:
            ...
            self.general_encoder = MyEncoder_ResNet(
                input_channel,
                output_channel=z_level_1_channel,
