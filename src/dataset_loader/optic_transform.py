...
def collate_fn_w_transform(batch):
    ...
    origin_image = np.array(normalize_image(image))
    origin_label = np.array(to_one_hot_list(label))

    data_dict = {'data': image, 'mask': label, 'name': name}
    tr_transforms = get_train_transform()
    data_dict = tr_transforms(**data_dict)
    label = np.array(to_one_hot_list(data_dict['mask']))
    image = np.array(normalize_image(data_dict["data"]))

    return {
        "image": torch.from_numpy(image).to(dtype=torch.float32),
        "label": torch.from_numpy(label).to(dtype=torch.float32),
        "origin_image": torch.from_numpy(origin_image).to(dtype=torch.float32),
        "origin_label": torch.from_numpy(origin_label).to(dtype=torch.float32),
        "pid": list(data_dict["name"])
    }
...
def collate_fn_wo_transform(batch):
    ...
    label = np.array(to_one_hot_list(label))
    # data_dict['mask'] = to_one_hot_list(data_dict['mask'])
    return {
        "image": torch.from_numpy(image).to(dtype=torch.float32),
        "label": torch.from_numpy(label).to(dtype=torch.float32),
        # "origin_image": torch.from_numpy(image).to(dtype=torch.float32),
        # "origin_label": torch.from_numpy(label).to(dtype=torch.float32),
        "pid": list(name)
    }
